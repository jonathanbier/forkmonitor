...
xml.rss version: '2.0' do
  xml.channel do
    ...
    xml.link feeds_ln_uncoops_url + "?page=#{@page}", rel: 'self'
    xml.link "#{feeds_ln_uncoops_url}?page=1", rel: 'first'
    xml.link feeds_ln_uncoops_url + "?page=#{@page + 1}", rel: 'next' if @page < @page_count
    xml.link feeds_ln_uncoops_url + "?page=#{@page - 1}", rel: 'previous' if @page > 1
    xml.link feeds_ln_uncoops_url + "?page=#{@page_count}", rel: 'last'
