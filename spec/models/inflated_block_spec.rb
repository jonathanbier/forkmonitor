...
RSpec.describe InflatedBlock do
  ...
  describe 'InflatedBlock.check_inflation!' do
    before do
      ...
      allow(Node).to receive(:with_mirror).and_return [@node]

      # throw the first time for lacking a comparison block
      expect { described_class.check_inflation!({ max: 0 }) }.to raise_error(InflatedBlock::TooFarBehindError)
    ...
    it "skips mirror node that's not synced" do
      ...
      described_class.check_inflation!({ max: 1 })
    end
    ...
    it 'calls gettxoutsetinfo on mirror node' do
    ...
    describe 'Mirror node has three more blocks' do
      before do
        @node.client.generate(2)
        test.sync_blocks
      end

      it 'fetches intermediate blocks' do
        described_class.check_inflation!({ max: 10 })
    ...
    describe 'Mirror node has a valid-headers tip' do
      before do
    ...
    describe 'Mirror node has a valid-fork' do
      before do
        test.disconnect_nodes(2, 0)
        test.disconnect_nodes(2, 1)
        expect(@node_b.client.getpeerinfo.count).to eq(0)
    ...
    describe 'with extra inflation' do
      ...
      it 'sends an alert' do
        expect { described_class.check_inflation!({ max: 10 }) }.to (change do
      ...
      it 'sends email only once' do
        ...
        expect { described_class.check_inflation!({ max: 10 }) }.not_to(change do
          ActionMailer::Base.deliveries.count
        end)
      end
    end
