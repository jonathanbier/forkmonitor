  ...
  def index_coin
    ...
    if stale?(etag: latest.try(:updated_at), last_modified: latest.try(:updated_at))
      render json: Rails.cache.fetch("Chaintip.#{params[:coin].downcase}.index.json") {
        @chaintips = Chaintip.where(status: "active", coin: @coin, parent_chaintip: nil).includes(:block, :node).order("blocks.work desc, nodes.client_type asc, nodes.name asc, nodes.version desc")
