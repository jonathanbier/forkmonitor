namespace 'blocks' do :env
  ...
  desc "Fetch missing version bits"
  task :fetch_version_bits => :environment do
    node = Node.where(coin: "BTC").reorder(version: :desc).first
    block = node.block
    while true
      if !block.version.present?
        puts "Fetch version for block #{ block.height }"
  ...
  desc "Version bit alerts - rules before Bitcoin Core 0.18"
  ...
  task :fetch_ancestors, [:height] => :environment do |action, args|
    until_height = args.height.to_i
    block = Node.where(coin: "BTC").reorder(version: :desc).first.block
    Node.where(coin: "BTC").reorder(version: :desc).first.find_block_ancestors!(block, until_height, true)
  end
end
